...
BEGIN
    ...
    SELECT
        ...
        (SELECT STRING_AGG(A2.фамилия + N' ' + A2.имя +
                           ISNULL(N' ' + A2.отчество, N''), N', ')
    ...
    FROM Книга К
    INNER JOIN Автор_Книга АК ON К.id_Ресурс = АК.id_Ресурс
    INNER JOIN Автор А ON АК.id_Автор = А.id_Автор
    ...
    INNER JOIN Физический_Экземпляр ФЭ ON ПР.id_Представление = ФЭ.id_Представление
